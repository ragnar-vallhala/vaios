...
// Bus arbiter for a shared peripheral (I2C / SPI / UART). One v_bus_t per
...
//
//   async  v_bus_submit(): returns at once. When the job reaches the bus its
//          start() kicks a DMA transfer; the DMA-complete ISR must call
//          v_bus_done_isr(), which runs done() and starts the next job straight
//          from the ISR (no context switch between back-to-back transfers).
//   sync   v_bus_lock()/v_bus_unlock(): the caller queues like any job, sleeps
//          until granted, then does its blocking transfer in its own task.
//   cyclic v_bus_cyclic_add(): the job is re-submitted every `period` calls of
//          v_bus_tick_isr(), which the board wires to a hardware timer IRQ.
//
// NavHAL DMA callbacks take no context, so each bus needs a trampoline:
//   static v_bus_t i2c1;
//   static void i2c1_dma_done(void) { v_bus_done_isr(&i2c1, 0); }
//   static void i2c1_timer(void)    { v_bus_tick_isr(&i2c1); }
// A zero-initialised v_bus_t is ready to use.
...
typedef struct v_bus_job v_bus_job_t;
struct v_bus_job {
  // --- set by the user ---
  ...
  int (*start)(v_bus_job_t *job);
  // Optional completion hook, usually in ISR context. May resubmit the job.
  void (*done)(v_bus_job_t *job, int rc);
  void *arg;
  uint8_t prio;    // higher is served first
  uint32_t period; // cyclic only: v_bus_tick_isr() calls between submissions
  // --- private ---
  v_bus_job_t *next;
  v_bus_job_t *cyc_next;
  SemaphoreHandle_t grant; // non-NULL only for a v_bus_lock() waiter
  uint32_t countdown;
  volatile uint8_t state;
};

typedef struct {
  v_bus_job_t *queue;         // pending, sorted by prio (desc), FIFO within
  v_bus_job_t *active;        // async job on the bus, NULL otherwise
  volatile uint8_t locked;    // held by a v_bus_lock() caller
  v_bus_job_t *cyclic;        // registered cyclic jobs
...
// must first stop the DMA / reset the peripheral, so no late v_bus_done_isr()
// can arrive and release the NEXT job. VA_FAIL if idle or held by v_bus_lock.
