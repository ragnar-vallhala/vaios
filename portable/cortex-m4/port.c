...
typedef struct {
  ...
  void print_registers(ExceptionStackFrame *frame) {
    ...
    // Optional: simple backtrace by scanning stack for plausible return
    // addresses
        ...
        for (uint32_t i = 0; i < stack_size; i++) {
          ...
          void hardfault_handler_c(ExceptionStackFrame * frame,
            ...
            __attribute__((naked)) void HardFault_Handler(void) {
              ...
              void hardfault_handler_entry(uint32_t * stack_pointer,
                                           uint32_t lr_unused, uint32_t dummy) {
                             uint32_t dummy) {
...
#define PENDSVSET (1U << 28)
                             void task_yield(void) {
                               ...
                               __attribute__((naked)) void scheduler_start(
                                   void) {
                                 __attribute__((naked)) void scheduler_start(
                                     void) {
                                   __asm volatile(
...
#define TCB_SP_OFF ((int)offsetof(TCB, sp))
                                 ...
                                 __attribute__((naked)) void PendSV_Handler(
                                   ...
                                   __attribute__((naked)) void SVCall_Handler(
                                       void) {
                                     __attribute__((naked)) void SVCall_Handler(
                                         void) {
                                       __asm volatile(
                                     ...
                                     void init_task_stack(TCB * task) {
                                       void init_task_stack(TCB * task) {
                                         ...
                                         *sp = ((uint32_t)task->entry) &
                                         ...
                                         sp--;
                                         *sp =
                                             (uint32_t)TASK_EXIT; // Hardware LR
